...
Проект **не рассчитан** на рядовых проектировщиков: обычному пользователю достаточно знать, что выгрузка происходит автоматически;
максимум — уметь запустить готовый батник.
...
- Учёт истории в `admin_data/history/history.xlsx`:
  ...
  - фиксировать дату/время последней проверки/выгрузки
...
- **Python:** CPython 3.12+ (проверены 3.12–3.13 вместе с `openpyxl`).
...
1. **CPython-оркестратор (`main.py`)**:
   ...
   - для каждой задействованной версии Revit формирует:
     - текстовый файл задач `Task<версия>.txt`,
...
3. **Результат**:
   ...
   Оркестратор сохраняет `history.xlsx` после каждого запуска даже в dry-run
   и при ошибках отдельных версий Revit. Итоговый статус «успех» означает,
   что файл истории записан и все запущенные pyRevit-сеансы отработали без
   ошибок (если pyRevit не запускался, проверяется только сохранение истории).

...
Назначение:
    ...
    - Даёт аккуратный, читаемый вывод в консоль:
        ...
        * вывод трассировок исключений под сообщением при наличии
          exc_info/stack_info в записи лога.
...
class ConsoleFormatter(logging.Formatter):
    ...
    Назначение:
        ...
        - Добавляет:
            ...
            * стек исключения/вызова под сообщением при наличии данных
              в LogRecord (exc_info/stack_info).
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        Формат:
            ...
            "<TAG> " (цветом уровня)
            "<сообщение>"
            [при наличии exc_info/stack_info] стек под сообщением
        ...
        # Добавляем стек исключения/вызова, если он присутствует в record.
        # Стек печатается отдельными строками под основным сообщением.
        extra_parts: list[str] = []
        if record.exc_info:
            extra_parts.append(self.formatException(record.exc_info))
        if record.stack_info:
            extra_parts.append(self.formatStack(record.stack_info))

        if extra_parts:
            msg = "\n".join([msg, *extra_parts])

...
Контракты:
    ...
    - Глобальная __models__ от pyRevit внутри этого скрипта
      не используется как основной источник путей; при необходимости может
      быть задействована отдельным отладочным кодом.

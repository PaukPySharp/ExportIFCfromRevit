...
- **`core/manage.py`**  
  ...
  - проверка существования JSON-конфига и файла маппинга на диске (если нужные файлы не найдены, выбрасывается ошибка, загрузка manage.xlsx и процесс экспорта прекращаются — нужно исправить пути к файлам в manage.xlsx и запустить экспорт заново);
...
- **`config/constants.py`**  
  ...
  - глобальные константы проекта:
    ...
    - имена файлов логов (`LOGFILE_OPENING_ERRORS`, `LOGFILE_MISSING_VIEW_TEMPLATE`, `LOGFILE_VERSION_NOT_FOUND` и т.п.),
      ...
      поэтому фактическое имя лога включает 3D-вид (например, `2_not_view_Export_IFC_in_models`);

...
- **[2. Обзор архитектуры](#2-обзор-архитектуры)**  
  ...
  - [2.2. Компоненты CPython-части](#22-компоненты-cpython-части)  
  - [2.3. Компоненты pyRevit-части и пакет `revit/`](#23-компоненты-pyrevit-части-и-пакет-revit)  
  - [2.4. Основные файлы данных и потоки](#24-основные-файлы-данных-и-потоки)  
  - [2.5. Что важно запомнить из архитектуры](#25-что-важно-запомнить-из-архитектуры)  

- **[3. Быстрый старт](#3-быстрый-старт)**  
  - [3.1. Что должно быть готово до начала](#31-что-должно-быть-готово-до-начала)  
  - [3.2. Подготовка репозитория и установка openpyxl](#32-подготовка-репозитория-и-установка-openpyxl)  
  - [3.3. Краткая проверка установки pyRevit](#33-краткая-проверка-установки-pyrevit)  
  - [3.4. Минимальная настройка `_settings/settings.ini`](#34-минимальная-настройка-_settingssettingsini)  
  - [3.5. Подготовка конфигов IFC на основе примера](#35-подготовка-конфигов-ifc-на-основе-примера)  
  - [3.6. Подготовка `admin_data` и `manage.xlsx`](#36-подготовка-admin_data-и-managexlsx)  
  - [3.7. Проверка требований к моделям Revit](#37-проверка-требований-к-моделям-revit)  
  - [3.8. Первый ручной запуск](#38-первый-ручной-запуск)  
  - [3.9. Подключение к запуску по расписанию (в общих чертах)](#39-подключение-к-запуску-по-расписанию-в-общих-чертах)  
...
- **[4. Требования к окружению и подготовка софта](#4-требования-к-окружению-и-подготовка-софта)**  
  ...
  - [4.2. Revit и IFC Exporter](#42-revit-и-ifc-exporter)  
  - [4.3. Сторонние плагины Revit](#43-сторонние-плагины-revit)  
  - [4.4. Python 3.12+: выбор и установка](#44-python-312-выбор-и-установка)  
  - [4.5. pyRevit: версии и общие требования](#45-pyrevit-версии-и-общие-требования)  
  - [4.6. Обновление компонентов и совместимость](#46-обновление-компонентов-и-совместимость)  
  - [4.7. Итог по окружению](#47-итог-по-окружению)  
  - [4.8. Полезный софт (по желанию)](#48-полезный-софт-по-желанию)  
...
- **[5. Структура проекта и назначение папок](#5-структура-проекта-и-назначение-папок)**  
  ...
  - [5.2. Папки с кодом: `core/`, `revit/`, `config/`, `utils/`](#52-папки-с-кодом-core-revit-config-utils)  
  - [5.3. Служебные настройки и bat-скрипты: `_settings/`](#53-служебные-настройки-и-bat-скрипты-_settings)  
  - [5.4. Рабочие данные: `admin_data/`](#54-рабочие-данные-admin_data)  
  - [5.5. Примеры и документация по IFC: `_examples/`](#55-примеры-и-документация-по-ifc-_examples)  
  - [5.6. Дополнительные служебные папки: `_for_python/`, `_git_images/`, `_docs/`](#56-дополнительные-служебные-папки-_for_python-_git_images-_docs)  
  - [5.7. Итог по структуре](#57-итог-по-структуре)  

- **[6. Настройка `_settings/settings.ini`](#6-настройка-_settingssettingsini)**  
  - [6.1. Общие принципы редактирования](#61-общие-принципы-редактирования)  
  - [6.2. `[Paths]`: пути к папкам](#62-paths-пути-к-папкам)  
  - [6.3. `[Files]`: файловые настройки](#63-files-файловые-настройки)  
  - [6.4. `[Settings]`: общие флаги и режимы](#64-settings-общие-флаги-и-режимы)  
  - [6.5. `[Revit]`: версии Revit и 3D-вид](#65-revit-версии-revit-и-3d-вид)  
  - [6.6. `[Excel]`: листы `manage.xlsx` и `history.xlsx`](#66-excel-листы-managexlsx-и-historyxlsx)  
  - [6.7. `[Mapping]`: структура папок конфигов IFC](#67-mapping-структура-папок-конфигов-ifc)  
  - [6.8. Дополнительные параметры и эволюция `settings.ini`](#68-дополнительные-параметры-и-эволюция-settingsini)  
  - [6.9. Пример минимальной конфигурации `_settings/settings.ini`](#69-пример-минимальной-конфигурации-_settingssettingsini)  
...
- **[7. Структура конфигов IFC](#7-структура-конфигов-ifc)**  
  ...
  - [7.3. Папка `01_Export_Layers`: маппинг категорий/слоёв](#73-папка-01_export_layers-маппинг-категорийслоёв)  
...
- **[10. Запуск и сценарии эксплуатации](#10-запуск-и-сценарии-эксплуатации)**  
  ...
  - [10.2. Ручной запуск через `run_export_ifc.bat`](#102-ручной-запуск-через-run_export_ifcbat)  
...
- **[11. Диагностика и устранение неполадок](#11-диагностика-и-устранение-неполадок)**  
  ...
  - [11.2. Проблема: bat-файл не запускается или мгновенно закрывается](#112-проблема-bat-файл-не-запускается-или-мгновенно-закрывается)  
  - [11.3. Проблема: Python/pyRevit не найден или падает на старте](#113-проблема-pythonpyrevit-не-найден-или-падает-на-старте)  
  - [11.4. Проблема: IFC-файлы не появляются или не обновляются](#114-проблема-ifc-файлы-не-появляются-или-не-обновляются)  
  - [11.5. Проблема: конкретные модели не попадают в выгрузку](#115-проблема-конкретные-модели-не-попадают-в-выгрузку)  
  - [11.6. Проблема: Revit зависает или выдаёт диалоги при автоматическом запуске](#116-проблема-revit-зависает-или-выдаёт-диалоги-при-автоматическом-запуске)  
  - [11.7. Что можно править руками, а чего лучше не трогать](#117-что-можно-править-руками-а-чего-лучше-не-трогать)  
  - [11.8. Использование dry-run для диагностики](#118-использование-dry-run-для-диагностики)  
...
- **[12. Рекомендации и связанные документы](#12-рекомендации-и-связанные-документы)**  
  ...
  - [12.4. Вектор развития и дальнейшие улучшения](#124-вектор-развития-и-дальнейшие-улучшения)  
...
#### 1.3.1 Если вы настраиваете всё с нуля
...
#### 1.3.2 Если «раньше всё работало, а теперь нет»
...
#### 1.3.3 Если вы разработчик, дорабатывающий ExportIFC
...
6. **Обновление истории и финальные логи**  
   ...
   - обновляет `history.xlsx` (факт последней «проверки/экспорта» по каждой модели) **всегда** — даже при dry-run (`run_pyrevit=False`) или если отдельные версии Revit завершились с ошибкой;
   - считает запуск успешным только если сохранился `history.xlsx` **и** pyRevit-выгрузка для всех версий не вернула ошибок (в режимах без запуска pyRevit — dry-run или отсутствие задач — успех определяется только сохранением `history.xlsx`);
   - записывает сводки по версиям и проблемным моделям;
   - добавляет разделители только в те текстовые логи pyRevit, которые реально менялись в этом запуске (чтобы в `_logs` было видно блоками по запускам без лишних пустых строк).
...
- вызывает `orchestrator.run()` и возвращает код возврата процесса:
  - `0`, если `orchestrator.run()` вернул успех (pyRevit отработал без ошибок
    или не запускался в dry-run, и удалось записать `history.xlsx`);
...
- **`core/exporter.py` → `ExportOrchestrator`**  
  ...
  - возвращает успех, только если pyRevit не сообщил об ошибках **и** файл истории удалось записать.
...
- **`core/manage.py`**  
  ...
  Если сам файл `<MANAGE_NAME>.xlsx` отсутствует, загрузчик сразу выбрасывает
  `FileNotFoundError`, и оркестратор прекращает работу до появления файла на месте.
  ...
  - проверка существования JSON-конфига и файла маппинга на диске (если нужные файлы не найдены, выбрасывается `FileNotFoundError`; в `main.py` это перехватывается как фатальная ошибка файловой валидации с коротким CRITICAL-сообщением, а полная трассировка выводится только при `DEBUG = True`; после исправления путей в manage.xlsx запуск нужно повторить);
...
- **`config/constants.py`**  
  ...
  - глобальные константы проекта:
    ...
    - формат даты/времени;
    - имена файлов логов (`LOGFILE_OPENING_ERRORS`, `LOGFILE_MISSING_VIEW_TEMPLATE`, `LOGFILE_VERSION_NOT_FOUND` и т.п.),
      причём `LOGFILE_MISSING_VIEW_TEMPLATE` — это шаблон для лога моделей без нужного 3D-вида;
      подстрока `$$$` заменяется на текущее значение `export_view3d_name` из `settings.ini` при формировании имени файла,
      поэтому фактическое имя лога включает 3D-вид (например, `2_not_view_Export`);
...
2. **Проверьте содержимое `admin_data`.**
   ...
   - папка `history` — создаётся автоматически при необходимости;
     - файл `history.xlsx` появится при первом сохранении истории, поэтому можно
       оставить пустую папку из шаблона или просто указать путь в настройках;
     - текстовый README из шаблона можно сохранить для напоминания;
   - папка `_logs` — также создаётся автоматически при запуске, но можно оставить
     подготовленную папку из шаблона.
...
- `enable_unmapped_export` — включать ли дополнительную выгрузку без маппинга свойств.
  ...
  - если `enable_unmapped_export = True`, но для конкретной строки колонки E и/или F заполнены  
   не полностью (пустая одна из них), строка при разборе `manage.xlsx` пропускается целиком:  
   модель **не попадает** в список задач на экспорт ни с маппингом, ни без маппинга;
...
1. **Неверный `dir_export_config` или сломанная структура папок IFC-конфигов.**
   ...
   - **Как проявляется:**
     - при старте выводится короткое сообщение об отсутствующем пути/файле (без «простыни» трассировки); подробная трассировка появляется только при `DEBUG = True`;
...
2. **Неверный `dir_admin_data`.**
   ...
   - **Как проявляется:**
     - при запуске ExportIFC сразу в консоли и общем логе появляется короткое CRITICAL-сообщение о том, что не найден `manage.xlsx` или `history.xlsx`; развёрнутая трассировка выводится только в режиме `DEBUG`;
...
1. По колонке A формирует список моделей `.rvt` для обработки.
   ...
   Берутся только файлы с **ровно одним расширением** `.rvt`: любые копии с
   хвостами (`.ifc.0001.RVT`, `.backup.rvt` и т.п.) или вовсе без нужного
   расширения игнорируются и в список задач не попадут.
...
**Важно**: если по указанному пути нет основного JSON (`<config_json>.json`), оркестратор завершится с фатальной ошибкой (`FileNotFoundError`) ещё на этапе разбора строки `manage.xlsx` — запуск не продолжится, пока файл не появится на месте.
...
Если `enable_unmapped_export` выключен, колонку можно оставить пустой. Если включён `enable_unmapped_export`, но заполнена только одна из колонок E или F, строка считается некорректной и будет пропущена целиком — модель не попадёт ни в основную выгрузку с маппингом, ни в дополнительную выгрузку без маппинга.
...
Этот JSON используется как настройки IFC-экспорта для nomap-выгрузки. Если колонка F пустая (или `enable_unmapped_export` выключен), nomap-экспорт для данной строки не выполняется. Если включён `enable_unmapped_export`, но заполнена только одна из колонок E или F, строка считается некорректной и будет пропущена целиком — модель не попадёт ни в основную выгрузку с маппингом, ни в дополнительную выгрузку без маппинга.  

**В таком случае обе колонки E и F должны быть заполнены, чтобы экспортировать модель без маппинга.**
...
Внутри `dir_admin_data` создаётся папка `_logs`. В неё ExportIFC пишет текстовые логи, которые помогают понять, что пошло не так **при подготовке задач на стороне CPython и при экспорте внутри Revit**. Папка создаётся автоматически через `ensure_dir` при запуске оркестратора, поэтому вручную её создавать не требуется; если `_logs` была удалена, она появится заново на следующем запуске.
...
Логи 1, 2 и 5 формируются внутри Revit (pyRevit-частью) во время экспорта. В конце каждого прогона в них дописывается строка-разделитель, чтобы можно было глазом отделить блоки разных запусков в пределах одного дня; разделитель ставится только если соответствующий лог менялся в текущем запуске, так что пустые файлы не плодятся лишними строками. Логи 3, 4 и 6 формируются на стороне CPython ещё до запуска Revit: это проблемы определения версии файлов и чтения дат модификации.
...
4. **Наблюдать вывод в консоли.**
   ...
   При серьёзных ошибках (нет `manage.xlsx`, не найден `settings.ini`, нет доступа к папкам)
   выводится короткое CRITICAL-сообщение (расширенный стек появляется только при `DEBUG = True`).
...
4. **Папка `admin_data/_logs`.**
   ...
   В конце файла может появиться строка-разделитель для отделения нескольких запусков в один день, но она добавляется только если соответствующий лог менялся в текущем запуске.
